...
from common.compat import LLMCache, load_json, save_json, topic_dir, variant_dir


SYSTEM_PROMPT = """You decide whether two META-CLUSTERS are duplicate broader research areas.
A meta-cluster groups several fine-grained topic clusters under one navigation-level umbrella.
Return only valid JSON:
{
  "is_duplicate": false,
  "reason": "brief explanation"
}
Mark duplicate only if the two meta-clusters cover essentially the same broad area for navigation.
Do NOT mark duplicate when clusters merely share a wide field (e.g. both are "NLP") but differ in sub-area, task family, or method paradigm."""


def fine_topic_summary(reps: list[dict], limit: int = 8) -> str:
    seen: set[str] = set()
    lines: list[str] = []
    for r in reps:
...
Fine topics included:
{fine_topic_summary(reps_a)}
...
Fine topics included:
{fine_topic_summary(reps_b)}
Representative titles:
{titles_b}

Are A and B duplicate broader research areas?"""
...
def run_variant(
    ...
    if out_path.exists() and not force:
        print(f"[skip] {name} meta distinctness exists; use --force")
        return
    ...
    n_dup = 0
    if workers <= 1:
        for i, j, sim in tqdm(pairs, desc=f"meta distinct {name}", leave=False):
        ...
        with ThreadPoolExecutor(max_workers=workers) as pool:
            ...
            for future in tqdm(as_completed(futures), total=len(futures), desc=f"meta distinct {name}", leave=False):
                row = future.result()
                n_dup += int(row["is_duplicate"])
                rows.append(row)
    result = {
        ...
        "prompt_version": "meta_distinctness_v1",
        "pairs": rows,
    }
    save_json(out_path, result)
    print(f"[done] {name}: meta pairs={len(rows)}, duplicate_rate={result['duplicate_pair_rate']}")
